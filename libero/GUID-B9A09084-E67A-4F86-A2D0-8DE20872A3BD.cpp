...
// By default, this example is run on a smaller image for faster co-simulation.
// If you want to run this example with a larger image, please comment out the
// line #define FAST_COSIM in define.hpp.
...
int main() {
    ...
    for (i = 0; i < HEIGHT; i++) {
        for (j = 0; j < WIDTH; j++) {
            ...
            canny_fifo(input_fifo, output_fifo);
            input_channel++;
        }
    }
